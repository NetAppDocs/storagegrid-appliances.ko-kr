...
.시작하기 전에
* 있습니다. link:verify-component-to-replace-sg6000cn.html["CMOS 배터리를 교체해야 하는 제품을 확인했습니다"]
* link:locating-controller-in-data-center.html["SG6000-CN 컨트롤러를 물리적으로 위치했다"]데이터 센터의 CMOS 배터리를 교체할 수 있습니다.
+

NOTE: 랙에서 어플라이언스를 분리하기 전에 가 link:power-sg6000-cn-controller-off-on.html["제품의 제어된 셧다운"] 필요합니다.

* 모든 케이블 및 을(를) 분리했습니다. link:reinstalling-sg6000-cn-controller-cover.html["제품 덮개를 분리했습니다"]
...
CAUTION: 개체의 복사본을 하나만 만드는 ILM 규칙을 사용한 적이 있는 경우 이 절차를 수행하는 동안 해당 개체에 일시적으로 액세스할 수 없게 될 수 있으므로 예약된 유지 관리 기간 동안 배터리를 교체해야 합니다. 에 대한 정보를 참조하십시오 https://docs.netapp.com/us-en/storagegrid-118/ilm/why-you-should-not-use-single-copy-replication.html["단일 복사본 복제를 사용하지 않아야 하는 이유"^].
