...
CAUTION: 드라이브의 데이터를 보존하려면 SSD 드라이브를 실패한 어플라이언스에서 차지했던 것과 동일한 드라이브 슬롯에 삽입하세요.  그렇지 않으면 Appliance Installer에서 경고가 표시되고 드라이브를 올바른 슬롯에 넣고 어플라이언스를 재부팅해야 어플라이언스가 그리드에 다시 합류할 수 있습니다.
